...
describe('SedLockedWarningComponent', () => {
  ...
  it('calls pool.try_import and emits importSuccess when Import Again button is clicked', async () => {
    jest.spyOn(spectator.component.importSuccess, 'emit');

    const importButton = await loader.getHarness(MatButtonHarness.with({ text: 'Import Again' }));
    await importButton.click();

    expect(spectator.inject(DialogService).jobDialog).toHaveBeenCalled();
    expect(spectator.component.importSuccess.emit).toHaveBeenCalled();
  });
});
